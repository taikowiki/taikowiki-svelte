...
```
- params
    - `after`: 특정 시간 이후로 업데이트된 곡의 데이터를 가져올 수 있습니다. (ms 단위 unix 시간)
...
곡 제목, 별칭 등을 검색합니다. 최대 20개의 결과를 반환합니다.
...
로그인 여부와 ip주소 또는 유저 닉네임, UUID를 가져옵니다.
...
```
- params
    - `all`: `true`인 경우 전체 데이터를 가져옵니다. 그렇지 않으면 상위 50개만 가져옵니다.
