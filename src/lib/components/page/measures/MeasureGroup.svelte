<script lang="ts">
    ...
    const subGroupedMeasures = groupBy(measures, measure => measure['상수']);

    const GroupTitle = createSSC<{group: number; opened:boolean;}>('div', ({group, opened}) => `
...
<GroupTitle {group} {opened} on:click={() => {opened = !opened;}}>
    {group}
</GroupTitle>
{#if opened}
